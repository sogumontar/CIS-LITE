...
class Pegawai extends \yii\db\ActiveRecord
{
    ...
    public function getIzinBawahan(){
        $izin = AtasanIzin::find()->where('cist_atasan_izin.deleted!=1')->andWhere(['cist_atasan_izin.pegawai_id' => $this->pegawai_id])->joinWith(['permohonanIzin.statusIzin' => function($query){
                $query->where('cist_status_izin.deleted!=1')->andWhere(['cist_status_izin.status_by_atasan' => 1]);
        }])->orderBy(['cist_atasan_izin.created_at' => SORT_ASC])->all();
        return $izin;
    }

    public function getCutiNontahunanBawahan(){
        $cuti_non = AtasanCutiNontahunan::find()->where('cist_atasan_cuti_nontahunan.deleted!=1')->andWhere(['cist_atasan_cuti_nontahunan.pegawai_id' => $this->pegawai_id])->joinWith(['pmhnnCutiNthn.statusCutiNontahunan' => function($query){
                $query->where('cist_status_cuti_nontahunan.deleted!=1')->andWhere(['cist_status_cuti_nontahunan.status_by_atasan' => 1]);
        }])->orderBy(['cist_atasan_cuti_nontahunan.created_at' => SORT_ASC])->all();
        return $cuti_non;
    }

    public function getCutiTahunanBawahan(){
        $cuti = AtasanCutiTahunan::find()->where('cist_atasan_cuti_tahunan.deleted!=1')->andWhere(['cist_atasan_cuti_tahunan.pegawai_id' => $this->pegawai_id])->joinWith(['pmhnnCutiThn.statusCutiTahunan' => function($query){
                $query->where('cist_status_cuti_tahunan.deleted!=1')->andWhere(['cist_status_cuti_tahunan.status_by_atasan' => 1]);
        }])->orderBy(['cist_atasan_cuti_tahunan.created_at' => SORT_ASC])->all();
        return $cuti;
    }
}
