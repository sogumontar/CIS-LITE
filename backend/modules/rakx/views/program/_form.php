...
<div class="program-form">
    ...
    <?php /*$form->field($model, 'diusulkan_oleh',[
               'horizontalCssClasses' => ['wrapper' => 'col-sm-8',]
        ])->dropDownList(
            ArrayHelper::map($pengusul, 'pejabat_id', function($pengusul){ return $pengusul->pegawai->nama.' - '.$pengusul->strukturJabatan->jabatan; }),['options' => [$pengusul[0]->pejabat_id => ['Selected'=>'selected']],])->label('Pengusul')
    */?>
    ...
    <?= $form->field($model, 'struktur_jabatan_has_mata_anggaran_id',[
            ...
            ArrayHelper::map($struktur_jabatan_has_mata_anggaran, 'struktur_jabatan_has_mata_anggaran_id', function($model){return $model->struktur_jabatan_has_mata_anggaran_id!=-999?($model->strukturJabatan->jabatan.' - '.$model->mataAnggaran->kode_anggaran.' '.$model->mataAnggaran->name):'------';}),["prompt"=>"--- Mata Anggaran ---", 'options' => [-999 => ['disabled' => true]], 'disabled' => /*$model->isNewRecord?*/false/*:true*/])->label('Mata Anggaran')
    ?>
