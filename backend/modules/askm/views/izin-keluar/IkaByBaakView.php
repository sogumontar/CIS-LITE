...
$this->params['breadcrumbs'][] = ['label' => 'Izin Keluar', 'url' => ['ika-by-baak-index']];
...
<div class="izin-keluar-view">
    ...
    <?php
        if ($model->status_request_dosen_wali == 2 && $model->status_request_keasramaan == 2 && $model->status_request_baak == 1) {
    ?>
    ...
    <div class="pull-right">
        ...
        <?= $uiHelper->renderButtonSet([
                ...
                'buttons' => [
                    'approve' => ['url' => Url::toRoute(['approve-by-baak', 'id' => $model->izin_keluar_id, 'id_baak' => $baak->pegawai_id]), 'label'=> 'Setujui Request', 'icon'=>'fa fa-check'], // id keasramaan diambil saat sudah login
                    'reject' => ['url' => Url::toRoute(['reject-by-baak', 'id' => $model->izin_keluar_id, 'id_baak' => $baak->pegawai_id]), 'label'=> 'Tolak Request', 'icon'=>'fa fa-times'],
                ],
